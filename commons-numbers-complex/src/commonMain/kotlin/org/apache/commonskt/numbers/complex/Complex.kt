 ...
 */
package org.apache.commonskt.numbers.complex
...
class Complex
    ...
    fun proj(): Complex {
        return if (isInfinite) {
            Complex(Double.POSITIVE_INFINITY, 0.0.withSign(imaginary))
        } else this
    }
    ...
    fun addImaginary(addend: Double): Complex {
        return Complex(real, imaginary + addend)
    }
    ...
    fun subtractImaginary(subtrahend: Double): Complex {
        return Complex(real, imaginary - subtrahend)
    }
    ...
    fun subtractFrom(minuend: Double): Complex {
        return Complex(minuend - real, -imaginary)
    }
    ...
    fun subtractFromImaginary(minuend: Double): Complex {
        return Complex(-real, minuend - imaginary)
    }
    ...
    fun multiply(factor: Double): Complex {
        return Complex(real * factor, imaginary * factor)
    }
    ...
    fun multiplyImaginary(factor: Double): Complex {
        return Complex(-imaginary * factor, real * factor)
    }
    ...
    fun divideImaginary(divisor: Double): Complex {
        return Complex(imaginary / divisor, -real / divisor)
    }
        ...
        if (real.isInfinite()) {
            ...
            zeroOrInf = if (real < 0) {
                if (!imaginary.isFinite()) {
                    ...
                    return Complex(0.0, 0.0.withSign(imaginary))
                }
                ...
                if (!imaginary.isFinite()) {
                    return Complex(real, Double.NaN)
                }
    ...
    private fun log(
        ...
        if (x == 0.0) {
            ...
            return constructor.invoke(
                ...
                if (negative(real)) PI.withSign(imaginary) else imaginary
            )
        }
        var re: Double
        ...
        if (x > HALF && x < ROOT2) {
            ...
            re = ln1p(x2y2m1(x, y)) * logOfeOver2
        } else {
 ...
 * DR 471: Complex math functions cacosh and ctanh](http://www.open-std.org/jtc1/sc22/wg14/www/docs/n1892.htm#dr_471).
    ...
    fun acosh(): Complex {
        ...
        return if (imaginary.isNaN() && real == 0.0) {
            Complex(Double.NaN, PI_OVER_2)
        } else acos(
            real,
            imaginary,
            { re: Double, im: Double ->  // Set the sign appropriately for real >= 0
                if (negative(im)) // Multiply by I
                    Complex(-im, re) else  // Multiply by -I
                    Complex(im, -re)
            }
        )
    }
 ...
 * inverse hyperbolic tangent](http://mathworld.wolfram.com/InverseHyperbolicTangent.html) of this complex number.
    ...
    companion object {
        ...
        private val NAN = Complex(Double.NaN, Double.NaN)
        ...
        fun ofPolar(rho: Double, theta: Double): Complex {
            ...
            if (!theta.isFinite() || negative(rho) || rho.isNaN()) {
                return NAN
            }
        ...
        private fun divide(re1: Double, im1: Double, re2: Double, im2: Double): Complex {
            ...
            if (getMaxExponent(a, b) > DoubleConsts.MAX_EXPONENT - 2) {
                ilogbw -= 2
                a /= 4.0
                b /= 4.0
            }
        ...
        private fun sqrt(real: Double, imaginary: Double): Complex {
            ...
            if (real.isNaN() || imaginary.isNaN()) {
                ...
                if (imaginary.isInfinite()) {
                    return Complex(Double.POSITIVE_INFINITY, imaginary)
                }
                return if (real.isInfinite()) {
                    ...
                    } else Complex(Double.POSITIVE_INFINITY, Double.NaN)
                } else NAN
            }
            ...
            if (inRegion(x, y, DoubleConsts.MIN_NORMAL, SQRT_SAFE_UPPER)) {
                // No over/underflow
                t = sqrt(2 * (abs(x, y) + x))
            } else {
                ...
                if (isPosInfinite(y)) {
                    ...
                    return if (real == Double.NEGATIVE_INFINITY) {
                        Complex(0.0, Double.POSITIVE_INFINITY.withSign(imaginary))
                    } else Complex(Double.POSITIVE_INFINITY, 0.0.withSign(imaginary))
                } else if (y == 0.0) {
                    // Real only
                    val sqrtAbs: Double = sqrt(x)
                    return if (real < 0) {
                        Complex(0.0, sqrtAbs.withSign(imaginary))
                    } else Complex(sqrtAbs, imaginary)
                } else if (x == 0.0) {
                    ...
                    return Complex(sqrtAbs, sqrtAbs.withSign(imaginary))
                } else {
                    // Over/underflow.
        ...
        private fun asin(
            ...
            if (x.isNaN()) {
                ...
                re =
                    if (isPosInfinite(y)) PI_OVER_4 else PI_OVER_2
                im = x
        ...
        private fun acos(
            ...
            return constructor.invoke(
                if (negative(real)) PI - re else re,
        ...
        private fun sinh(
            ...
            return if (x > SAFE_EXP) {
                ...
                coshsinh(x, real, imaginary, true, constructor)
        ...
        private fun cosh(
            ...
            if (real == 0.0) {
                ...
                return if (imaginary.isFinite()) {
                    ...
                    constructor.invoke(
                        cos(imaginary),
                        changeSign(real, sin(imaginary))
                    )
                } else constructor.invoke(Double.NaN, changeSign(real, imaginary))
            ...
            if (imaginary == 0.0) {
                ...
                return constructor.invoke(cosh(real), changeSign(imaginary, real))
            }
            val x: Double = abs(real)
            return if (x > SAFE_EXP) {
                // Approximate sinh/cosh(x) using exp^|x| / 2
                coshsinh(x, real, imaginary, false, constructor)
            } else constructor.invoke(
        ...
        private fun tanh(
            ...
            // Handle inf or nan.
            if (!isPosFinite(x) || !imaginary.isFinite()) {
                if (isPosInfinite(x)) {
                    if (imaginary.isFinite()) {
            ...
            if (x.isNaN()) {
                ...
                if (inRegion(x, y, SAFE_LOWER, SAFE_UPPER)) {
                    ...
                    if (x >= SAFE_UPPER) {
                        ...
                        // log1p(4x / (x^2 + y^2))
                        re = if (isPosInfinite(x) || isPosInfinite(y)) {
                            0.0
        ...
        private fun x2y2m1(x: Double, y: Double): Double {
            ...
            if (x < 1 && xx + yy > 0.5) {
                ...
                val xHigh = splitHigh(x)
                val xLow = x - xHigh
                val yHigh = splitHigh(y)
                val yLow = y - yHigh

                // Accurate split multiplication x * x and y * y
                val x2Low = squareLow(xLow, xHigh, xx)
                val y2Low = squareLow(yLow, yHigh, yy)
        ...
        private fun sumx2y2m1(
            ...
            // q=running sum
            var q = x2Low - 1
            var e1 = fastSumLow(-1.0, x2Low, q)
            var e3 = q + x2High
            var e2 = sumLow(q, x2High, e3)

            // Grow expansion of f1 into e
            q = y2Low + e1
            e1 = sumLow(y2Low, e1, q)
            var p = q + e2
            e2 = sumLow(q, e2, p)
            var e4 = p + e3
            e3 = sumLow(p, e3, e4)
        ...
        private fun changeSign(magnitude: Double, signedValue: Double): Double {
            return if (negative(signedValue)) -magnitude else magnitude
        }
        ...
        private fun x2y2(x: Double, y: Double): Double {
            ...
            val xHigh = splitHigh(x)
            val xLow = x - xHigh
            val xxLow = squareLow(xLow, xHigh, xx)
            // Dekker mul12
            val yHigh = splitHigh(y)
            val yLow = y - yHigh
            val yyLow = squareLow(yLow, yHigh, yy)
            // Dekker add2
            val r = xx + yy
