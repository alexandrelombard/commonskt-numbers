...
object LogBeta {
    ...
    private fun deltaMinusDeltaSum(
        ...
        if (a < 0 ||
            ...
            throw GammaException(GammaException.OUT_OF_RANGE(a, 0.0, b))
        }
        if (b < TEN) {
            throw GammaException(GammaException.OUT_OF_RANGE(b, TEN, Double.POSITIVE_INFINITY))
        }
        val h = a / b
        val p = h / (1 + h)
        val q = 1 / (1 + h)
    ...
    private fun sumDeltaMinusDeltaSum(
        ...
        if (p < TEN) {
            throw GammaException(GammaException.OUT_OF_RANGE(p, TEN, Double.POSITIVE_INFINITY))
        }
        if (q < TEN) {
            throw GammaException(GammaException.OUT_OF_RANGE(q, TEN, Double.POSITIVE_INFINITY))
        }
    ...
    private fun logGammaMinusLogGammaSum(
        ...
        if (a < 0) {
            throw GammaException(GammaException.OUT_OF_RANGE(a, 0.0, Double.POSITIVE_INFINITY))
        }
        if (b < TEN) {
            throw GammaException(GammaException.OUT_OF_RANGE(b, TEN, Double.POSITIVE_INFINITY))
        }
