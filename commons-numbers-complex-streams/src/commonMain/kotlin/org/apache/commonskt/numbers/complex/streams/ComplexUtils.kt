...
package org.apache.commonskt.numbers.complex.streams

import org.apache.commonskt.numbers.complex.Complex
...
object ComplexUtils {
    ...
    fun polar2Complex(r: Double, theta: Double): Complex {
        if (r < 0) {
            throw NegativeModulusException(r)
        }
    ...
    fun polar2Complex(r: DoubleArray, theta: DoubleArray): Array<Complex> {
        ...
        val c: Array<Complex> = Array(length) { x ->
            if (r[x] < 0) {
                throw NegativeModulusException(r[x])
            }
            Complex.ofCartesian(
                r[x] * cos(theta[x]),
                r[x] * sin(theta[x]))
    ...
    fun complex2Interleaved(
        ...
        if (interleavedDim > 1 || interleavedDim < 0) {
            throw IndexOutOfRangeException(interleavedDim)
        }
        val w = c.size
        val h: Int = c[0].size
